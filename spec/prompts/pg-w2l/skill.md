...
# SkillDistill
signature: episode -> skill_graph?: GraphArtifact

If the episode demonstrates a repeatable procedure, emit a skill graph artifact
(nodes with signatures, edges with ports). If not, emit null.
The artifact is a candidate. You do not register it into the live skill index.

Output: { "skill": null | { "id": string, "nodes": object[], "edges": object[] } }
