...
# QueryForm
signature: slots, goal -> query, indexes[]

Turn working-memory slots and the active goal into a retrieval query.
Name indexes (episodic, semantic, skill, structural) as data, not as a conversation.

Output: { "query": { "text": string, "indexes": string[] } }
